...
- **`token_expired`** → request a fresh token: `curl -X POST https://app.jinn.works/api/agents/request-demo-token -H 'content-type: application/json' -d '{"skill":"programmatic-seo-planner"}'`, update `JINN_MCP_TOKEN`, retry.
